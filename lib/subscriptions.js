  ...
  if (existing) {
    ...
    await db.run(
      `INSERT INTO dispatch_subscriptions
         (stripe_customer_id, stripe_subscription_id, email, plan, status, is_test, current_period_end, created_at, updated_at)
       VALUES (?, ?, ?, ?, 'active', 0, ?, ?, ?)`,
...
/**
 * Admin test-access grant. Gives a driver the full paid-client experience
 * (PAID badge, broadcasts, route matching) WITHOUT a Stripe payment, for
 * testing. The row is flagged is_test=1 and labeled TEST everywhere in
 * admin; it is excluded from revenue accounting. Refuses when the email
 * already has a real Stripe subscription record (test access must never be
 * mixed with real payment data). A later real Stripe payment converts the
 * test grant via upsertActive (is_test cleared).
 */
async function grantTestAccess({ email, plan }) {
  const e = cleanEmail(email);
  const p = String(plan || '').toLowerCase();
  if (!e) throw new Error('Email is required.');
  if (p !== 'basic' && p !== 'complete') throw new Error('Choose a plan: basic or complete.');
  const now = Date.now();
  const existing = await getByEmail(e);
  if (existing && (existing.stripe_customer_id || existing.stripe_subscription_id)) {
    throw new Error('This email already has a real Stripe subscription record — test access cannot be mixed with real payment data.');
  }
  if (existing) {
    await db.run(
      `UPDATE dispatch_subscriptions
       SET email = ?, plan = ?, status = 'active', is_test = 1,
    ...
    await db.run(
      `INSERT INTO dispatch_subscriptions
         (stripe_customer_id, stripe_subscription_id, email, plan, status, is_test, created_at, updated_at)
       VALUES (NULL, NULL, ?, ?, 'active', 1, ?, ?)`,
      [e, p, now, now]
    );
  }
  await tagLead(e, 'DISPATCH_SUBSCRIBER');
  await tagLead(e, p === 'basic' ? 'DISPATCH_BASIC' : 'DISPATCH_COMPLETE');
  await recordEvent(e, 'dispatch_test_access_granted', p === 'basic' ? 'transitnow-basic' : 'transitnow-complete', {
    plan: p, test: true,
  });
  console.log('[subscriptions] test access granted', { email: e, plan: p });
  return getByEmail(e);
}

/**
 * Revoke an admin test-access grant. Only ever touches is_test rows —
 * real Stripe subscriptions can only be changed by Stripe webhook events.
 */
async function revokeTestAccess({ email }) {
  const e = cleanEmail(email);
  const existing = await getByEmail(e);
  if (!existing || !existing.is_test) {
    throw new Error('No test access grant found for this driver.');
  }
  const now = Date.now();
  ...
  await recordEvent(existing.email, 'dispatch_test_access_revoked', null, { test: true });
  console.log('[subscriptions] test access revoked', { email: existing.email });
