...
/**
 * Record the first Room nurture touchpoint. The stage only advances to
 * CONTACTED when the lead has not already moved past it — a real browser
 * lands on /room/offer (INTERESTED) before the immediate nurture email
 * sends, and stages never move backward. The contact itself is always
 * logged with the FIRST_TOUCH_SENT tag.
 */
async function markContacted(leadId) {
  if (!leadId) return null;
  await tags.addTag(leadId, 'FIRST_TOUCH_SENT');
  const current = await getStage(leadId);
  if (!current || rankOf(current) < rankOf('CONTACTED')) {
    return setStage(leadId, 'CONTACTED');
  }
  return current;
}

module.exports = { STAGES, tagFor, getStage, setStage, markContacted };
