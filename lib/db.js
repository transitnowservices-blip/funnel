...
const SCHEMA_STATEMENTS = [
  ...
  // --- Referral milestone payouts (two-sided, Veho-style) -----------------------
  // One row per (attribution, side). side = 'referrer' | 'referred'.
  // status: pending (subscription active, milestone not yet met) -> earned
  // (milestone met) -> paid (admin marked paid). void = subscription lapsed
  // before the milestone; never paid.
  `CREATE TABLE IF NOT EXISTS referral_payouts (
     ...
     milestone       TEXT NOT NULL DEFAULT 'paid_30d',
  ...
  `CREATE INDEX IF NOT EXISTS idx_referral_payouts_attr ON referral_payouts (attribution_id)`,
  ...
  // --- Content library: Davena's reusable flyers/messages, organized by offer.
  ...
  `CREATE TABLE IF NOT EXISTS content_attribution (
     ...
     lead_id     INT NOT NULL,
     created_at  INT,
     UNIQUE (content_id, lead_id)
   )`,
  `CREATE INDEX IF NOT EXISTS idx_content_attr_lead ON content_attribution (lead_id)`,
  `CREATE INDEX IF NOT EXISTS idx_content_attr_content ON content_attribution (content_id)`,
];
