...
async function getCommandMetrics(raw = {}) {
  ...
  const [
    ...
    // Phase 6: community (J), service plans (K), alerts, follow-ups wired in.
    openAlerts, followupsDue, communityReportsOpen, planRequestsPending,
  ] = await Promise.all([
    ...
    // Phase 6 — point-in-time counts from the real tables (honestly labeled).
    db.get(`SELECT COUNT(*) c FROM alerts WHERE status = 'open'`).then((r) => Number(r.c)),
    db.get(`SELECT COUNT(*) c FROM (
        SELECT MAX(id) mid FROM lead_followups GROUP BY lead_id
      ) m JOIN lead_followups f ON f.id = m.mid
      WHERE f.followup_status <> 'CLOSED' AND f.next_follow_up_at IS NOT NULL AND f.next_follow_up_at <= ?`,
      [Date.now()]).then((r) => Number(r.c)),
    db.get(`SELECT COUNT(*) c FROM community_reports WHERE status = 'open'`).then((r) => Number(r.c)),
    db.get(`SELECT COUNT(*) c FROM driver_plan_changes
            WHERE event = 'requested'
              AND id IN (SELECT MAX(id) FROM driver_plan_changes GROUP BY driver_id)`).then((r) => Number(r.c)),
  ]);
  ...
  const cards = [
    ...
    // Phase 6: community + service plans wired in (existing pages, deep links);
    // alerts and follow-ups surfaced from real tables.
    { key: 'open_alerts', label: 'Open alerts', value: openAlerts, href: '/admin/alerts', kind: 'state' },
    { key: 'followups_due', label: 'Follow-ups due', value: followupsDue, href: '/admin/alerts', kind: 'state' },
    { key: 'community_reports_open', label: 'Community reports (open)', value: communityReportsOpen, href: '/admin/community', kind: 'state' },
    { key: 'plan_requests_pending', label: 'Plan requests (pending)', value: planRequestsPending, href: '/admin/plans', kind: 'state' },
  ];
