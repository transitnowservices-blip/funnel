...
async function createOrUpdateDriver(clean) {
  ...
  if (existing) {
    ...
    const updated = await getDriverById(existing.id);
    // Every driver gets a referral code automatically (live from signup).
    try { await require('./referrals').ensureCodeForDriver(updated); } catch (e) { /* non-blocking */ }
    return updated;
  }
  ...
  const created = await getDriverById(info.lastInsertRowid);
  // Every new driver gets a referral code automatically (live from signup).
  try { await require('./referrals').ensureCodeForDriver(created); } catch (e) { /* non-blocking */ }
  return created;
}
...
// --- Email templates ----------------------------------------------------------
function onboardDriverEmail(driver, dashUrl, referral = null) {
  const refBlock = referral && referral.code ? `
<p><strong>💰 Earn ${esc(referral.referrerBonus)} per driver you refer:</strong> your personal referral link is live right now — <a href="${esc(referral.link)}">${esc(referral.link)}</a>. When someone you refer becomes a paid TransitNow dispatch client and stays active ${esc(String(referral.milestoneDays))} days, you earn <strong>${esc(referral.referrerBonus)}</strong> and they earn <strong>${esc(referral.referredBonus)}</strong>. Track your referrals and earnings on the <strong>Referrals</strong> tab of your dashboard.</p>` : '';
...
<span style="color:#555">Save this link — it's your private access to your routes, packages, support, community, and referrals. No password needed.</span></p>${refBlock}
...
// Session request/accept/record lifecycle. HONESTY IS ABSOLUTE HERE:
// provider_ref is stored only when a real provider creates a room (never
// faked); every surface must label unconfigured-provider workflows
// "SIMULATED TEST" and show "VIDEO PROVIDER REQUIRED". Recording is OFF by
// default and requires explicit opt-in BEFORE it may start. The driver's
// camera/mic never activate without their explicit confirmation.
// Events, participants, and messages are APPEND-ONLY — history is never edited.
const SESSION_STATUSES = ['REQUESTED', 'ACCEPTED', 'LIVE', 'ENDED', 'MISSED', 'DECLINED'];
...
const SESSION_PRIORITIES = ['NORMAL', 'HIGH', 'URGENT'];
const SESSION_PRIORITY_LABELS = { NORMAL: 'Normal', HIGH: 'High', URGENT: 'Urgent' };
// Session reasons — EXACTLY these, per the spec.
const SESSION_REASONS = [
  'route_issue', 'package_issue', 'delivery_issue', 'wrong_address', 'customer',
...
// Valid lifecycle transitions. Anything else is rejected with an error.
const SESSION_TRANSITIONS = {
  REQUESTED: ['ACCEPTED', 'DECLINED', 'MISSED'],
  ACCEPTED: ['LIVE', 'ENDED', 'DECLINED'],
  LIVE: ['ENDED'],
  ENDED: [],
  MISSED: [],
  DECLINED: [],
...
const SESSION_EVENT_TYPES = [
  'requested', 'accepted', 'declined', 'missed', 'started', 'ended',
  'joined', 'left', 'message', 'consent', 'toggle', 'note',
  'recording_consent', 'recording_access',
];
const SESSION_EMERGENCY_NOTE =
  'If this is an emergency, call 911 first. A live support session is not a substitute for emergency services.';
const SESSION_COVERAGE_NOTE =
  'Requests can be submitted any time. We do not staff 24/7 human support — response depends on operations coverage.';

/** TN-LIVE-000123 style public session IDs. */
async function nextSessionId() {
  const n = await nextSequence('live-session');
  return `TN-LIVE-${String(n).padStart(6, '0')}`;
}
async function nextLiveRequestId() {
  const n = await nextSequence('live-request');
  return `TN-LIVEREQ-${String(n).padStart(6, '0')}`;
}
async function nextTrainingEventId() {
  const n = await nextSequence('live-training');
  return `TN-LIVETRN-${String(n).padStart(6, '0')}`;
}

function validateSessionRequest(input) {
  const errors = [];
  const clean = {
    reason: String(input.reason || '').trim(),
    priority: String(input.priority || 'NORMAL').trim().toUpperCase(),
    notes: String(input.notes || '').trim().slice(0, 2000),
    route_id: input.route_id ? Number(input.route_id) : null,
    contract_id: input.contract_id ? Number(input.contract_id) : null,
    share_location: [true, 'on', '1', 1].includes(input.share_location) ? 1 : 0,
  };
  if (!SESSION_REASONS.includes(clean.reason)) errors.push('Choose a reason for the session.');
  if (!SESSION_PRIORITIES.includes(clean.priority)) clean.priority = 'NORMAL';
  if (clean.route_id && !Number.isFinite(clean.route_id)) clean.route_id = null;
  if (clean.contract_id && !Number.isFinite(clean.contract_id)) clean.contract_id = null;
  return { ok: errors.length === 0, errors, clean };
}

/** Append-only session event. */
async function logSessionEvent(sessionId, eventType, actor, details) {
  if (!SESSION_EVENT_TYPES.includes(eventType)) throw new Error(`Unknown session event: ${eventType}`);
  const s = await getSession(sessionId);
  if (!s) throw new Error('Session not found');
  await db.run(
    `INSERT INTO live_session_events (session_id, event_type, actor, details, ts) VALUES (?, ?, ?, ?, ?)`,
    [sessionId, eventType, String(actor || 'system').slice(0, 40), String(details || '').slice(0, 2000), Date.now()]
  );
}

async function getSessionEvents(sessionId) {
  return db.all('SELECT * FROM live_session_events WHERE session_id = ? ORDER BY ts ASC, id ASC', [sessionId]);
}

/**
 * Create a live session request: live_support_requests row + live_sessions
 * row (REQUESTED) + initial event + ops notification. The notification is
 * queued (email_queue), never claimed as delivered. provider_ref stays NULL
 * until a real provider creates a room — it is never faked.
 */
async function requestLiveSession({ driverId, reason, priority = 'NORMAL', notes = '', routeId = null, contractId = null, shareLocation = 0 }) {
  const driver = await getDriverById(driverId);
  if (!driver) throw new Error('Driver not found');
  const { ok, errors, clean } = validateSessionRequest({
    reason, priority, notes, route_id: routeId, contract_id: contractId, share_location: shareLocation,
  });
  if (!ok) throw new Error(errors.join(' '));
  const sessionId = await nextSessionId();
  const requestId = await nextLiveRequestId();
  const now = Date.now();
  await db.run(
    `INSERT INTO live_sessions
       (session_id, driver_id, route_id, contract_id, priority, reason, status,
        consent_camera, consent_mic, share_location, notes, provider_ref, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, 'REQUESTED', 0, 0, ?, ?, NULL, ?, ?)`,
    [sessionId, driverId, clean.route_id, clean.contract_id, clean.priority, clean.reason,
     clean.share_location, clean.notes || null, now, now]
  );
  await db.run(
    `INSERT INTO live_support_requests (request_id, driver_id, session_id, reason, priority, status, created_at)
     VALUES (?, ?, ?, ?, ?, 'open', ?)`,
    [requestId, driverId, sessionId, clean.reason, clean.priority, now]
  );
  // Recording row exists from birth with consent OFF (explicit opt-in later).
  await db.run(
    `INSERT INTO live_session_recordings (session_id, consent_given, created_at) VALUES (?, 0, ?)`,
    [sessionId, now]
  );
  await logSessionEvent(sessionId, 'requested', 'driver',
    `Driver requested a live session: ${SESSION_REASON_LABELS[clean.reason]} (priority ${clean.priority}). ${clean.share_location ? 'Driver is sharing location.' : 'Location sharing is off.'}`);
  await notifyOps(
    `${clean.priority === 'URGENT' ? 'URGENT ' : ''}Live session requested: ${sessionId} — ${SESSION_REASON_LABELS[clean.reason]}`,
    `<p><strong>${clean.priority === 'URGENT' ? 'URGENT ' : ''}Live session request</strong> from driver ${esc(driver.full_name)} (${esc(driver.email)}).</p>` +
    `<p>Session: ${esc(sessionId)}<br>Reason: ${esc(SESSION_REASON_LABELS[clean.reason])}<br>Priority: ${esc(clean.priority)}<br>` +
    `${clean.notes ? `Notes: ${esc(clean.notes)}<br>` : ''}</p>` +
    `<p>Accept it in the admin live-sessions queue. ${esc(SESSION_COVERAGE_NOTE)}</p>`,
...
async function getSession(sessionId) {
  return db.get('SELECT * FROM live_sessions WHERE session_id = ?', [sessionId]);
}

async function listSessions({ status = null, driverId = null } = {}) {
  const where = [];
  const params = [];
  if (status) {
  ...
  if (driverId) { where.push('driver_id = ?'); params.push(driverId); }
  return db.all(
    `SELECT * FROM live_sessions ${where.length ? 'WHERE ' + where.join(' AND ') : ''} ORDER BY created_at DESC`,
    params
  );
}

/** The driver's currently open session, if any (REQUESTED/ACCEPTED/LIVE). */
async function getActiveSessionForDriver(driverId) {
  return db.get(
    `SELECT * FROM live_sessions WHERE driver_id = ? AND status IN ('REQUESTED','ACCEPTED','LIVE')
...
/**
 * Transition a session's status. Rejects invalid transitions — the lifecycle
 * cannot be skipped or rewound.
 */
async function setSessionStatus(sessionId, to, actor = 'system', note = '') {
  if (!SESSION_STATUSES.includes(to)) throw new Error(`Unknown session status: ${to}`);
  const s = await getSession(sessionId);
  if (!s) throw new Error('Session not found');
  const allowed = SESSION_TRANSITIONS[s.status] || [];
  if (!allowed.includes(to)) {
    throw new Error(`Cannot move session ${sessionId} from ${s.status} to ${to}.`);
  }
  const now = Date.now();
  const sets = ['status = ?', 'updated_at = ?'];
  const params = [to, now];
  if (to === 'LIVE' && !s.started_at) { sets.push('started_at = ?'); params.push(now); }
  if (to === 'ENDED' && !s.ended_at) { sets.push('ended_at = ?'); params.push(now); }
  params.push(sessionId);
  await db.run(`UPDATE live_sessions SET ${sets.join(', ')} WHERE session_id = ?`, params);
  if (to === 'ENDED' || to === 'MISSED' || to === 'DECLINED') {
    await db.run(`UPDATE live_support_requests SET status = 'resolved', resolved_at = ? WHERE session_id = ?`, [now, sessionId]);
    // Workflow participants leave with the session: stamp left_at on anyone still joined.
    await db.run(`UPDATE live_participants SET left_at = ? WHERE session_id = ? AND left_at IS NULL`, [now, sessionId]);
  }
  const eventType = { ACCEPTED: 'accepted', DECLINED: 'declined', LIVE: 'started', ENDED: 'ended', MISSED: 'missed' }[to];
  await logSessionEvent(sessionId, eventType, actor, note || `Session ${to.toLowerCase()}${actor === 'dispatcher' ? ' by operations' : ''}.`);
  return getSession(sessionId);
}

async function acceptSession(sessionId, dispatcherName, note = '') {
  const name = String(dispatcherName || '').trim().slice(0, 120) || 'Operations';
  const s = await setSessionStatus(sessionId, 'ACCEPTED', 'dispatcher', note || `Accepted by ${name}.`);
  await db.run('UPDATE live_sessions SET dispatcher_name = ? WHERE session_id = ?', [name, sessionId]);
  // Dispatcher joins the session workflow (acceptance = joining the session).
  const existing = await db.get(
    `SELECT id FROM live_participants WHERE session_id = ? AND role = 'dispatcher' AND staff_name = ? AND left_at IS NULL`,
    [sessionId, name]);
  if (!existing) await addSessionParticipant({ sessionId, staffName: name, role: 'dispatcher' });
  return getSession(sessionId);
}
async function declineSession(sessionId, dispatcherName, note = '') {
  return setSessionStatus(sessionId, 'DECLINED', 'dispatcher', note || `Declined by ${String(dispatcherName || 'Operations')}.`);
}
async function startSession(sessionId, actor = 'dispatcher') {
  return setSessionStatus(sessionId, 'LIVE', actor);
}
async function endSession(sessionId, actor = 'system', note = '') {
  return setSessionStatus(sessionId, 'ENDED', actor, note);
}
async function markSessionMissed(sessionId, actor = 'dispatcher', note = '') {
  return setSessionStatus(sessionId, 'MISSED', actor, note || 'No dispatcher response; marked missed.');
}

/** Driver explicitly confirms camera/mic activation. Nothing activates before this. */
async function recordDriverMediaConsent(sessionId, driverId, { camera = false, mic = false } = {}) {
  const s = await getSession(sessionId);
  if (!s) throw new Error('Session not found');
  if (Number(s.driver_id) !== Number(driverId)) throw new Error('Session not found');
  if (!['ACCEPTED', 'LIVE'].includes(s.status)) {
    throw new Error('Camera/mic consent can only be given once operations has accepted the session.');
  }
  const now = Date.now();
  // Explicit consent doubles as the driver joining the session workflow.
  const joined = await db.get(
    `SELECT id FROM live_participants WHERE session_id = ? AND role = 'driver' AND driver_id = ? AND left_at IS NULL`,
    [sessionId, driverId]);
  if (!joined) await addSessionParticipant({ sessionId, driverId, role: 'driver' });
  await db.run(
    `UPDATE live_sessions SET consent_camera = ?, consent_mic = ?, updated_at = ? WHERE session_id = ?`,
    [camera ? 1 : 0, mic ? 1 : 0, now, sessionId]
  );
  await logSessionEvent(sessionId, 'consent', 'driver',
    `Driver confirmed media consent: camera ${camera ? 'ON' : 'OFF'}, microphone ${mic ? 'ON' : 'OFF'}.`);
  return getSession(sessionId);
}

async function setLocationSharing(sessionId, driverId, on) {
  const s = await getSession(sessionId);
  if (!s) throw new Error('Session not found');
  if (Number(s.driver_id) !== Number(driverId)) throw new Error('Session not found');
  const now = Date.now();
  await db.run('UPDATE live_sessions SET share_location = ?, updated_at = ? WHERE session_id = ?', [on ? 1 : 0, now, sessionId]);
  await logSessionEvent(sessionId, 'toggle', 'driver', `Location sharing turned ${on ? 'ON' : 'OFF'} by driver (voluntary).`);
  return getSession(sessionId);
}

async function addSessionParticipant({ sessionId, driverId = null, staffName = null, role }) {
  const s = await getSession(sessionId);
  if (!s) throw new Error('Session not found');
  if (!['driver', 'dispatcher', 'observer'].includes(role)) throw new Error('Unknown participant role.');
  const now = Date.now();
  await db.run(
    `INSERT INTO live_participants (session_id, driver_id, staff_name, role, joined_at) VALUES (?, ?, ?, ?, ?)`,
    [sessionId, driverId, staffName, role, now]
  );
  await logSessionEvent(sessionId, 'joined', role === 'driver' ? 'driver' : 'dispatcher',
    `${role === 'driver' ? 'Driver' : `Operations (${staffName || 'staff'})`} joined the session.`);
  return getSessionParticipants(sessionId);
}

async function leaveSessionParticipant({ sessionId, participantId, actor = 'system' }) {
  const p = await db.get('SELECT * FROM live_participants WHERE id = ? AND session_id = ?', [participantId, sessionId]);
  if (!p) throw new Error('Participant not found');
  if (p.left_at) return getSessionParticipants(sessionId);
  await db.run('UPDATE live_participants SET left_at = ? WHERE id = ?', [Date.now(), participantId]);
  await logSessionEvent(sessionId, 'left', actor, `${p.role === 'driver' ? 'Driver' : `Operations (${p.staff_name || 'staff'})`} left the session.`);
  return getSessionParticipants(sessionId);
}

async function getSessionParticipants(sessionId) {
  return db.all('SELECT * FROM live_participants WHERE session_id = ? ORDER BY joined_at ASC, id ASC', [sessionId]);
}

/** In-session chat (append-only). Allowed while the session is open. */
async function addSessionMessage({ sessionId, senderType, senderId = null, message }) {
  const s = await getSession(sessionId);
  if (!s) throw new Error('Session not found');
  if (!['REQUESTED', 'ACCEPTED', 'LIVE'].includes(s.status)) {
    throw new Error('Messages can only be sent while the session is open.');
  }
  if (!['driver', 'dispatcher'].includes(senderType)) throw new Error('Unknown sender.');
  const text = String(message || '').trim().slice(0, 2000);
  if (!text) throw new Error('Message cannot be empty.');
  const now = Date.now();
  await db.run(
    `INSERT INTO live_session_messages (session_id, sender_type, sender_id, message, ts) VALUES (?, ?, ?, ?, ?)`,
    [sessionId, senderType, senderId, text, now]
  );
  await logSessionEvent(sessionId, 'message', senderType, `${senderType === 'driver' ? 'Driver' : 'Operations'} sent a chat message.`);
  return getSessionMessages(sessionId);
}

async function getSessionMessages(sessionId) {
  return db.all('SELECT * FROM live_session_messages WHERE session_id = ? ORDER BY ts ASC, id ASC', [sessionId]);
}

// --- Recording: OFF by default, explicit opt-in BEFORE recording may start ---
const RECORDING_RETENTION_MS = 90 * 24 * 3600 * 1000; // 90-day retention

async function getSessionRecording(sessionId) {
  return db.get('SELECT * FROM live_session_recordings WHERE session_id = ?', [sessionId]);
}

/**
 * Set recording consent. `consented` must be an explicit true — anything
 * else leaves recording OFF. Consent is logged (append-only) and access-logged.
 */
async function setRecordingConsent(sessionId, { consented, by }) {
  const rec = await getSessionRecording(sessionId);
  if (!rec) throw new Error('Recording record not found');
  const give = consented === true;
  const changed = (rec.consent_given === 1) !== give;
  const now = Date.now();
  await db.run(
    `UPDATE live_session_recordings
       SET consent_given = ?, consent_by = ?, consent_at = ?, retention_expires_at = ?
     WHERE id = ?`,
    [give ? 1 : 0, String(by || '').slice(0, 120) || null, give ? now : null, give ? now + RECORDING_RETENTION_MS : null, rec.id]
  );
  // Audit trail only on an actual state change: re-posting the same choice
  // (e.g. confirming OFF) must not fabricate consent activity.
  if (changed) {
    await logSessionEvent(sessionId, 'recording_consent', 'system',
      give ? `Recording consent given by ${by || 'unknown'}. Retention expires 90 days from consent.` : 'Recording consent withdrawn/not given — recording stays OFF.');
    await logRecordingAccess(rec.id, by || 'system', give ? 'recording consent granted' : 'recording consent declined');
  }
  return getSessionRecording(sessionId);
}

async function logRecordingAccess(recordingId, accessedBy, purpose) {
  await db.run(
    `INSERT INTO live_recording_access_log (recording_id, accessed_by, purpose, accessed_at) VALUES (?, ?, ?, ?)`,
    [recordingId, String(accessedBy || 'system').slice(0, 120), String(purpose || '').slice(0, 500), Date.now()]
  );
}

async function getRecordingAccessLog(recordingId) {
  return db.all('SELECT * FROM live_recording_access_log WHERE recording_id = ? ORDER BY accessed_at ASC, id ASC', [recordingId]);
}

async function getSupportRequest(requestId) {
  return db.get('SELECT * FROM live_support_requests WHERE request_id = ?', [requestId]);
}

// --- Live training events / attendance / content library ------------------------
function validateTrainingEvent(input) {
  const errors = [];
  const clean = {
    title: String(input.title || '').trim().slice(0, 200),
    description: String(input.description || '').trim().slice(0, 2000),
    scheduled_at: input.scheduled_at ? Number(input.scheduled_at) : null,
  };
  if (!clean.title) errors.push('Give the training event a title.');
  if (clean.scheduled_at && !Number.isFinite(clean.scheduled_at)) clean.scheduled_at = null;
  return { ok: errors.length === 0, errors, clean };
}

async function createTrainingEvent({ title, description = '', scheduledAt = null, createdBy = 'admin' }) {
  const { ok, errors, clean } = validateTrainingEvent({ title, description, scheduled_at: scheduledAt });
  if (!ok) throw new Error(errors.join(' '));
  const eventId = await nextTrainingEventId();
  const now = Date.now();
  await db.run(
    `INSERT INTO live_training_events (event_id, title, description, scheduled_at, provider_ref, status, created_by, created_at)
     VALUES (?, ?, ?, ?, NULL, 'scheduled', ?, ?)`,
    [eventId, clean.title, clean.description || null, clean.scheduled_at, createdBy, now]
  );
  return getTrainingEvent(eventId);
}

async function getTrainingEvent(eventId) {
  return db.get('SELECT * FROM live_training_events WHERE event_id = ?', [eventId]);
}

async function listTrainingEvents({ upcomingOnly = false } = {}) {
  const where = upcomingOnly ? `WHERE status = 'scheduled' AND (scheduled_at IS NULL OR scheduled_at >= ?)` : '';
  const params = upcomingOnly ? [Date.now() - 3600 * 1000] : [];
  return db.all(`SELECT * FROM live_training_events ${where} ORDER BY scheduled_at ASC, created_at DESC`, params);
}

async function recordTrainingAttendance(eventId, driverId, attended = true) {
  const ev = await getTrainingEvent(eventId);
  if (!ev) throw new Error('Training event not found');
  const driver = await getDriverById(driverId);
  if (!driver) throw new Error('Driver not found');
  const now = Date.now();
  await db.run(
    `INSERT INTO live_training_attendance (event_id, driver_id, attended, attended_at)
     VALUES (?, ?, ?, ?)
     ON CONFLICT(event_id, driver_id) DO UPDATE SET attended = excluded.attended, attended_at = excluded.attended_at`,
    [eventId, driverId, attended ? 1 : 0, now]
  );
  return getTrainingAttendance(eventId);
}

async function getTrainingAttendance(eventId) {
  return db.all('SELECT * FROM live_training_attendance WHERE event_id = ? ORDER BY attended_at ASC', [eventId]);
}

async function createTrainingContent({ title, description = '', url = '', durationSecs = null }) {
  const t = String(title || '').trim().slice(0, 200);
  if (!t) throw new Error('Give the training content a title.');
  const now = Date.now();
  const info = await db.run(
    `INSERT INTO video_training_content (title, description, url, duration_secs, created_at) VALUES (?, ?, ?, ?, ?)`,
    [t, String(description || '').slice(0, 2000) || null, String(url || '').slice(0, 1000) || null,
     durationSecs ? Number(durationSecs) : null, now]
  );
  return db.get('SELECT * FROM video_training_content WHERE id = ?', [info.lastInsertRowid]);
}

async function listTrainingContent() {
  return db.all('SELECT * FROM video_training_content ORDER BY created_at DESC', []);
}

// Phase 5 exports are attached after the definitions above (same pattern as Phase 2).
Object.assign(module.exports, {
  SESSION_STATUSES,
