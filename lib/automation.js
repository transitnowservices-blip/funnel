...
async function processOneEmail(row) {
  ...
  // Dispatcher password resets address the dispatcher directly (lead_id is
  // NULL). Guards: valid address, dispatcher account still exists and is
  // active, not suppressed. No lead record is required (dispatchers are
  // staff, not marketing leads). Never claims delivery beyond the provider.
  if (row.sequence === 'dispatch-reset') {
    return processDispatchResetEmail(row);
  }
  ...
  if (row.sequence === 'roomNurture' && row.step === 'room-nurture-1' && row.lead_id) {
    // First Room touchpoint delivered -> record the contact. The stage only
    // moves to CONTACTED if the lead hasn't already advanced (a real browser
    // hits /room/offer, which is INTERESTED, before this email sends).
    await pipeline.markContacted(row.lead_id);
  }
...
/**
 * Send one dispatcher password-reset email. Guards: valid address, the
 * dispatcher account still exists and is active (a dispatcher deactivated
 * after requesting the reset must not receive it), not suppressed. No lead
 * record is required (dispatchers are staff, not marketing leads). Never
 * claims delivery beyond the mail provider.
 */
async function processDispatchResetEmail(row) {
  const emailAddr = String(row.email || '').trim().toLowerCase();
  if (!emailAddr) return cancelRow(row.id, 'lead-missing');
  const d = await db.get('SELECT id, active FROM dispatchers WHERE email = ?', [emailAddr]);
  if (!d || d.active !== 1) return cancelRow(row.id, 'inactive-dispatcher');
  if (await isSuppressed(emailAddr)) return cancelRow(row.id, 'suppressed');
  await email.sendEmail({
    to: emailAddr,
    subject: row.subject,
 ...
 */
async function runSchedulerPass(now = Date.now()) {
  ...
  const noProgressNudges = await queueNoProgressNudges(now);
  // Referral milestone payouts: attribute new applications to codes, create
  // pending rows for active paid subscribers, flip pending -> earned at 30
  // continuous days, void lapses.
  let referralAttribution = { created: 0, updated: 0 };
  try { referralAttribution = await require('./referrals').runAttribution(); } catch (e) { /* non-blocking */ }
  let referralMilestones = { created: 0, earned: 0, voided: 0, checked: 0 };
  try { referralMilestones = await require('./referrals').checkMilestones(now); } catch (e) { /* non-blocking */ }
  const { sent, cancelled } = await processDueEmails(now);
  return { sent, cancelled, abandonedCarts: abandoned, checkinReminders, dailyNudges: nudges.email, dailyNudgeSms: nudges.sms, routeMatchCycle: routeMatches, assistantWeekly, roomAssistantWeekly, noProgressNudges, referralMilestones };
}
...
/**
 * Weekly no-progress nudge for Wealth Builder's Room members.
 * On Mondays, every ACTIVE claimed member with no weekly check-in in the
 * last 7 days gets one friendly email asking two questions: what did you
 * work on, and what will you complete next? Never shaming — just an
 * invitation to report in. Idempotent per Chicago week. Never mails
 * canceled, inactive, suppressed, or unclaimed members.
 */
async function queueNoProgressNudges(now = Date.now()) {
  try {
    const rm = require('./route_matching');
    if (!rm.isMondayChicago(now)) return { ran: false, reason: 'not-monday' };
    const weekKey = rm.chicagoWeekKey(now);
    ...
    for (const m of members) {
      ...
      try {
        const stats = await room.progressStats(emailAddr);
        if (stats && stats.lastCheckin) lastCheckinAt = Number(stats.lastCheckin.created_at) || null;
      } catch (err) { /* no check-ins yet — treat as no progress */ }
      if (lastCheckinAt && now - lastCheckinAt < 7 * 24 * 3600 * 1000) {
        skipped++;
        continue;
      }
      const dupe = await db.get(
        `SELECT 1 FROM email_queue WHERE email = ? AND sequence = 'room-noprogress-nudge' AND step = ? AND status IN ('queued','sent','provider-stub')`,
        [emailAddr, weekKey]
      );
      if (dupe) continue;
      const name = m.name ? String(m.name).split(' ')[0] : 'Wealth Builder';
      const checkinUrl = base ? `${base}/room/checkin` : '/room/checkin';
      const body = [
        `<p>Hi ${escHtmlLocal(name)},</p>`,
        `<p>No pressure and no judgment — I just noticed you haven't checked in this week, and I wanted to ask:</p>`,
        `<p><strong>1. What did you work on this week?</strong><br><strong>2. What will you complete next week?</strong></p>`,
        `<p>Two honest sentences are enough. Checking in keeps your 90-day goal moving — and there's always a fresh week to start.</p>`,
        `<p><a href="${escHtmlLocal(checkinUrl)}">Submit your weekly check-in</a></p>`,
        `<p style="color:#888;font-size:12px">Wealth Builder's Room. Guidance only — no guaranteed income, revenue, or results.</p>`,
      ].join('\n');
      await db.run(
        `INSERT INTO email_queue
           (lead_id, email, sequence, step, subject, body_html, product_id, scheduled_for, status)
         VALUES (NULL, ?, 'room-noprogress-nudge', ?, ?, ?, 'room', ?, 'queued')`,
        [emailAddr, weekKey, 'Quick check-in: what did you work on this week?', body, now]
      );
      queued++;
    }
    console.log(`[scheduler] room no-progress nudges: queued=${queued} skipped=${skipped} week=${weekKey}`);
    return { ran: true, queued, skipped, weekKey };
  } catch (err) {
    console.error('[scheduler] room no-progress nudges failed:', err.message);
    return { ran: false, reason: 'error', error: err.message };
  }
}
